...
public class UdfFindRoot {
    ...
    public static void main(String[] args) throws Exception {
        ...
        // 从 LVD 获取的 root ICB
        long rootIcb = 38825583;
        
        System.out.println("Partition starts at: " + partitionStart);
        System.out.println("Root ICB from LVD: " + rootIcb);
        
        // 方法1: 假设 ICB 是相对于 partition 的
        long rootSector = rootIcb;
        System.out.println("\nTrying root at sector " + rootSector + "...");
        
        byte[] rootDir = readSector(rootSector);
        if (rootDir != null && rootDir.length >= 200) {
            int tag = (rootDir[0] & 0xFF) | ((rootDir[1] & 0xFF) << 8);
            System.out.println("Tag at " + rootSector + ": " + tag);
            
            if (tag == 261) {
                // File Entry
                int fileType = rootDir[18] & 0xFF;
                long infoLen = readUInt64LE(rootDir, 72);
                System.out.println("File type: " + fileType + " (4=dir)");
                System.out.println("Info length: " + infoLen);
                
                // 解析 allocation descriptors 来找数据位置
                // 这里简化处理 - 直接列出目录内容
                System.out.println("\nListing directory entries:");
                listDirectory(rootDir, partitionStart);
            }
        }
        
        // 方法2: 直接扫描 partition 开始区域
        System.out.println("\n=== Scanning partition area for directories ===");
        for (int i = 0; i < 128; i++) {
            byte[] sec = readSector(partitionStart + i);
            ...
            if (tag == 261) {
                ...
                if (fileType == 4 && infoLen > 0) {
                    // 这是一个目录
                    System.out.println("Directory at sector " + (partitionStart + i) + 
                                     ", size=" + infoLen + " bytes");
                    
                    // 尝试读取目录内容
                    if (i < 10) {  // 只处理前几个
                        listDirectory(sec, partitionStart);
                    }
                }
            }
        }
    ...
    static void listDirectory(byte[] dirData, long baseSector) {
        // 解析 directory entries
        // 格式: 2 bytes length, ..., filename
        int pos = 0;
        int count = 0;
        
        while (pos < dirData.length - 36 && count < 20) {
